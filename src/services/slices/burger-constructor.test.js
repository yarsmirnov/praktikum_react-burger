...
describe('Test burger-constructor reducer', () => {
  it('addItem() should add an item to items', () => {
    ...
    expect(reducer(initial, addItem(item1)))
      .toEqual(expected1);
    expect(reducer(expected1, addItem(item2)))
      .toEqual(expected2);
  })


  it('setBun() should add item to items', () => {
    const initial = { items: [] };

    const expected = { items: [ bun1 ] };

    expect(reducer(initial, setBun(bun1)))
      .toEqual(expected);
  })


  it('setBun() should replace bun item in items', () => {
    const initial = {
      items: [ bun1, item1 ]
    };
    ...
    expect(reducer(initial, setBun(bun2)))
      .toEqual(expected);
  })

  it('removeItem() should remove correct item', () => {
    const initial = { items: [ bun1, item1, item2, item3 ] };
    const uuidToRemove = item2.uuid;

    const expected = { items: [ bun1, item1, item3] };

    expect(reducer(initial, removeItem(uuidToRemove)))
      .toEqual(expected);
  })

  it('moveItem() should correctly change order of items', () => {
    const initial = { items: [ bun1, item1, item2, item3 ] };
    ...
    expect(reducer(initial, moveItem(payload1)))
      .toEqual(expected1);
    expect(reducer(initial, moveItem(payload2)))
      .toEqual(expected2);
    expect(reducer(initial, moveItem(payload3)))
      .toEqual(expected3);
  })

  it('clearConstructor() should reset store to initial', () => {
    const initial = { items: [ bun1, item1, item2, item3 ] };

    const expected = { items: [] };

    expect(reducer(initial, clearConstructor()))
      .toEqual(expected);
  })
});
